...
# ---------------------------------------------------------------------------
# [FIX 1] KIIT NEXUS link — only append when the user is explicitly asking
#         about KIIT NEXUS itself, not just any KIIT topic.
# ---------------------------------------------------------------------------
...
# Sentence to strip if the LLM adds it when it shouldn't
_NEXUS_LINK_PATTERN = re.compile(
    r"\s*For more info on KIIT NEXUS,?\s*visit their official website:\s*https://kiitnexus\.in/?\s*\.?",
    re.IGNORECASE,
)
...
def strip_nexus_link(answer: str) -> str:
    """Remove the auto-appended KIIT NEXUS link from answers that don't need it."""
    return _NEXUS_LINK_PATTERN.sub("", answer).strip()
...
FEE RULES (IMPORTANT):
8. When the user asks about "fee", "fees", "cost", "charges", "price", or "how much" for ANY B.Tech programme (e.g., B.Tech CSE, Computer Science, IT), look for the fee table in the context. For example:
...
9. When asked about hostel fees, answer ONLY from the hostel fee data. Include room types, AC/Non-AC, occupancy, and mess fees.

SOCIETIES RULES (IMPORTANT):
10. "Technical societies at KIIT" or "societies at KIIT" or "clubs at KIIT" = answer about KSAC (KIIT Student Activity Centre) societies like KRS, Konnexions, IEEE, GDG, MLSA, CyberVault, AISoC, etc.
11. "Tech domains at KIIT NEXUS" or "KIIT NEXUS domains" = answer about KIIT NEXUS project domains: Web Development, Android Development, Flutter Development, Machine Learning.
12. KIIT NEXUS is a student-built application/chatbot — it is NOT a KIIT society. Do NOT confuse KIIT NEXUS domains with KIIT university societies.
    IMPORTANT: ONLY append the line "For more info on KIIT NEXUS, visit their official website: https://kiitnexus.in/" if the user's question is EXPLICITLY and SPECIFICALLY about KIIT NEXUS (e.g. asks about "KIIT NEXUS", "nexus app", "nexus domains", "nexus community"). Do NOT add this line for any other question, including general KIIT questions, SAP portal, societies, fees, or admissions.
...
def build_chain():
    ...
    # model_kwargs device pin removed: Streamlit Cloud's torch build raises
    # NotImplementedError on an explicit .to("cpu") cast. Let sentence-transformers
    # auto-detect the device instead.
    ...
    # [FIX] Switched from similarity → MMR for better retrieval diversity.
    # fetch_k=20 candidates, then MMR picks the best 6 with minimal redundancy.
