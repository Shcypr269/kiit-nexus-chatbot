    ...
    for txt_file in processed_path.rglob("*.txt"):
        ...
        for doc in docs:
            ...
            # 3. KIITEE file — split across multiple categories
                ...
                for chunk in file_chunks:
                    chunk_text = chunk.page_content.lower()
                    # Fee detection — includes hostel fees, seat rent, etc.
                    fee_keywords = ["fee structure", "per semester", "refund", "scholarship",
                                    "tuition", "fee", "seat rent", "electricity charges",
                                    "hostel admission", "boarding charge", "messing charge"]
                    has_fee_indicator = any(k in chunk_text for k in ["rs.", "per semester", "fee", "/-"])
                    # [FIX] University/history check must come BEFORE placements check.
                    # Chunks about KIIT's founding contain words like "1992", "1997",
                    # "industrial training" — they must not fall into placements.
                    if any(k in chunk_text for k in [
                        "founded", "founder", "achyuta samanta",
                        "1992", "1997", "started in", "industrial training institution",
                        "deemed university", "established"
                    ]):
                        chunk.metadata["category"] = "university"
                    elif any(k in chunk_text for k in fee_keywords):
                        chunk.metadata["category"] = "fees"
                    # Hostel chunk that mentions fees → tag as fees, not campus
                    elif "hostel" in chunk_text and has_fee_indicator:
                        chunk.metadata["category"] = "fees"
